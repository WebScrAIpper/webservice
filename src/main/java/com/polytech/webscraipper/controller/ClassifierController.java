...
public class ClassifierController {
  ...
  public ResponseEntity<String> addClassifier(@PathVariable String classifier) {
    ...
    if (!result) {
      return ResponseEntity.status(HttpStatus.BAD_REQUEST).body("The classifier already exists.");
    }
    return ResponseEntity.status(HttpStatus.OK).body("The classifier has been added.");
  }
}
